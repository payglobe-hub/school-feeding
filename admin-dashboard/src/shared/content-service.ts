...
function initializeContentService() {
  ...
  try {
    ...
    console.warn('Content service Firebase not initialized yet:', error);
...
try {
  ...
  console.log('Content service initialization deferred until config is loaded');
}
...
class ContentManagementService {
  ...
  async createContent(contentData: Omit<ContentItem, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
    ...
    try {
      ...
      const docRef = await addDoc(collection(db, this.collectionName), content);
      console.log('Content created:', docRef.id);
